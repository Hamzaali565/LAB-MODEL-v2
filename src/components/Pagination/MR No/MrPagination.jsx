...
const MrPagination = ({ department, sendToParent, resetFlag, length }) => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${url}/getmr`);
  ...
  const filterNames = (input) => {
    ...
    let d = data3.filter(
      (item) =>
        String(item.mrNo).toLowerCase().includes(searchTerm) ||
        item.patientName.toLowerCase().includes(searchTerm) ||
        item.cell.toLowerCase().includes(searchTerm)
    );
    if (input.length <= 0) {
      setData(data2);
    } else {
  ...
  return (
    <div className="border-2 flex flex-col">
      {data.length > 0 && (
        <h1 className=" flex justify-center underline my-2">PATIENT LIST</h1>
      )}
        ...
        <div className="flex justify-center">
          <table className="bg-gray-950">
            <tr className="border-2 items-center">
              <th className="hidden md:flex">MR NO.</th>
              <th width="400">Patient Name</th>
              <th width="400">Mobile No.</th>
            </tr>
            ...
            {data &&
              currentPageData.map((item, index) => (
                ...
                >
                  <th className="hidden md:flex">{item.mrNo}</th>
                  <th width="400">{item.patientName}</th>
                  <th width="200">{item.cell}</th>
                </tr>
              ))}
          </table>
        </div>
