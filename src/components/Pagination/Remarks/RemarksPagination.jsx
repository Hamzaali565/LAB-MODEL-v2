...
const TestPagination = ({ department, sendToParent, resetFlag, rerender }) => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${url}/getremarks`);
  ...
  return (
    <div className="border-2 flex flex-col">
      {data.length > 0 && (
        <h1 className=" flex justify-center underline my-2">REMARKS DATA</h1>
      )}
        ...
        <div className="flex justify-center">
          <table className="bg-gray-950">
            <tr className="border-2 items-center">
              <th className="hidden md:flex">Remark Code</th>
            ...
            {data &&
              currentPageData.map((item, index) => (
                ...
                >
                  <th className="hidden md:flex">{item.remarkCode}</th>
                  <th width="400">{item.remarks}</th>
                  <th width="200">{item.testName}</th>
                </tr>
              ))}
          </table>
        </div>
